...
const CreatorTaskSubmissionsScreen: React.FC = () => {
  ...
  const loadSubmissions = async () => {
    try {
      const data = await creatorService.getTaskSubmissions(filter);
      setSubmissions(data);
    } catch (error: any) {
      Toast.show({
        type: 'error',
  ...
  const renderSubmissionCard = ({ item }: { item: TaskSubmission }) => (
    <TouchableOpacity
      style={styles.card}
      onPress={() =>
        navigation.navigate(ROUTES.CREATOR_TASK_SUBMISSION_DETAILS, {
...
export default CreatorTaskSubmissionsScreen;
